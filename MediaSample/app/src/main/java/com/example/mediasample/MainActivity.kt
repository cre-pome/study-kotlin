...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 音声ファイルのURI文字列を作成。
        val mediaFIleStr = "android.resource://${packageName}/${R.raw.mountain_stream}"
        Log.i("mediasanmpme", "create mediaFIleStr :" + mediaFIleStr )
        // 音声ファイルのURI文字列を元にURIオブジェクトを生成。
        val mediaFileUri = Uri.parse(mediaFIleStr)

        try{
            // メディアプレーヤーに音声ファイルを指定
            _player?.setDataSource(applicationContext, mediaFileUri)

            // 非同期でのメディア再生準備が完了した際のリス名を設定
            ...
            Log.e("MediaSample", "メディアプレーヤー準備時の例外発生")
        }
